...
import { generateGroupCode, collectGroupImages, collectGroupTextObjects } from "./helpers/generate-group-code";
import { getSvgPathHelperCode } from "@/lib/svg-path-to-canvas";

/**
 * Generates complete, runnable Node.js code using the canvas package.
 * Orchestrates helper functions for each object type.
 *
 * @param state - Full editor state
 * @returns Self-contained CommonJS JavaScript file string
 */
export function generateNodeCanvasCode(state: EditorState): string {
...
function generateHeader(state: EditorState): string {
  ...
  const deps = hasQRCode ? "canvas qrcode" : "canvas";
...
function generateRequires(state: EditorState): string {
  const hasQRCode = state.objects.some((o) => o.type === "qrcode");
  let requires = `const { createCanvas, loadImage, registerFont } = require("canvas");
const fs = require("fs");
`;
  if (hasQRCode) {
